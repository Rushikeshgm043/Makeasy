...
function Validp() {
    ...
    // Check if password length is between 6-8 and contains at least one letter and one number
    if (val.length < 6 || val.length > 8 || !/[A-Za-z]/.test(val) || !/\d/.test(val)) {
        alert('Password should be 6-8 characters long and contain at least one letter and one number.');
